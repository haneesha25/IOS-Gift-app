...
class SignUPVC: UIViewController {
    ...
    private func setup() {
        ...
        if socialData != nil {
            self.txtFirstName.text = socialData.firstName
            self.txtLastName.text = socialData.lastName
    ...
    @IBAction func btnSignUpTapped(_ sender: UIButton) {
        ...
        if error.isEmpty {
            self.getExistingUser(fName: self.txtFirstName.text!.trim(), lName: self.txtLastName.text!.trim(), email: self.txtEmail.text!.trim(), code: self.txtCode.text!.trim(), mobile: self.txtMobile.text!.trim(), city: self.txtCity.text!.trim(), state: self.txtState.text!.trim(), zip: self.txtZipcode.text!.trim(), password: self.txtPassword.text!.trim())
        }else{
...
extension SignUPVC {
    ...
    func createAccount(fName: String, lName: String, email: String, code: String, mobile: String, city: String, state: String, zip: String, password: String) {
        ...
        {  err in
            if let err = err {
                ...
                GFunction.user = UserModel(docID: (ref?.documentID.description)!, fName: fName, lName: lName, code: code, mobile: mobile, city: city, state: state, zipcode: zip, email: email, password: password)
    ...
    func getExistingUser(fName: String, lName: String, email: String, code: String, mobile: String, city: String, state: String, zip: String, password: String) {
        ...
        _ = AppDelegate.shared.db.collection(fUser).whereField(fEmail, isEqualTo: email).addSnapshotListener{ querySnapshot, error in
            ...
            if snapshot.documents.count == 0 {
                self.createAccount(fName: fName, lName: lName, email: email, code: code, mobile: mobile, city: city, state: state, zip: zip, password: password)
                self.flag = true
            }else{
                if !self.flag {
